...
var BBY = {
	init: function(options){
		...
		this.buyingOptions = resource('buyingOptions', { key: this.options.key, debug: this.options.debug, url: 'https://api.bestbuy.com/beta', headers:this.options.headers});
		this.categories= resource('categories', this.options);
		this.products = resource('products', this.options);		
		this.recommendations = resource('recommendations', { key: this.options.key, debug: this.options.debug, url: 'https://api.bestbuy.com/beta', headers:this.options.headers});			
		this.reviews= resource('reviews', this.options);
		this.smartLists= resource('smartLists', { key: this.options.key, debug: this.options.debug, url: 'https://api.bestbuy.com/beta', headers:this.options.headers});
